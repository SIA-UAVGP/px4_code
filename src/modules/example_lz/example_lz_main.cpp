...
int example_lz_thread_main(int argc, char *argv[])
{
	...
	struct yaw_sp_calculated_s yaw_sp_pub;
	...
	struct obstacle_position_s obstacle_position_pub;
	...
	while(!thread_should_exit)
	{
		...
		if (updated_pos)
		{
			...
			// 发布主题，返回上述航向期望
			_yaw_sp_calculated_pub = orb_advertise(ORB_ID(yaw_sp_calculated), &yaw_sp_pub);
			orb_publish(ORB_ID(yaw_sp_calculated), _yaw_sp_calculated_pub, &yaw_sp_pub);
		}
		...
		if(updated_local_pos)
		{
			...
			// 发布主题，返回上述障碍物虚拟位置
			_obstacle_position_pub=orb_advertise(ORB_ID(obstacle_position), &obstacle_position_pub);
			orb_publish(ORB_ID(obstacle_position), _obstacle_position_pub, &obstacle_position_pub);
		}
